...
var Grid = function() {
  ...
  self.x      = 30;
  self.y      = 30;
  self.actual_width  = 205;
  self.actual_height = 293;
  // self.holesX = 25;
  // self.holesY = 36;
  self.update = function() {
    // self.x      = 30;
    // self.y      = 30;
    self.actual_width  = robot_data['grid']['width'];
    self.actual_height = robot_data['grid']['height'];

    if (self.object != undefined) {
      self.object.remove();
    }
    self.object = main_svg.image(robot_data['grid']['svg_url']);
  ...
  self.resize = function(width) {
    ...
    self.height = self.actual_height * self.scale;
...
var Skin = function() {
  ...
  self.update = function() {
    self.x = 0;
    self.y = 0;
    self.x_offset = robot_data['skin']['x_offset'];
    self.y_offset = robot_data['skin']['y_offset'];
    self.actual_width  = robot_data['skin']['width'];
    self.actual_height = robot_data['skin']['height'];

    if (self.object != undefined) {
      self.object.remove();
    }
    self.object = main_svg.image(robot_data['skin']['svg_url']);
    self.object.addClass('skin');
    self.object.draggable().on('dragmove', function(e) { e.preventDefault(); });
  };
  self.update();
  self.resize = function() {
    ...
    self.x = virtualModel.grid.x + mm_to_screen(robot_data['skin']['x_offset']);
    self.y = virtualModel.grid.y + mm_to_screen(robot_data['skin']['y_offset']);
    self.width  = mm_to_screen(self.actual_width);
    self.height = mm_to_screen(self.actual_height);

    self.object.move(self.x, self.y);
    self.object.size(self.width, self.height);
    self.object.front();
    console.log('skin front');
  };
  // self.resize();
};
...
var Dof = function(name) {
  ...
  self.set_value_tags = function(value, tags, update_call) {
    ...
    self.set_value(value, update_call);
    return true;
  };
...
var Module = function(config) {
  ...
  self.set_dof_tags = function(value, tags) {
    var return_value = false;
    for (var i = 0; i < self.dofs.length; i++) {
      var dof = self.dofs[i];
      if (dof.set_value_tags(value, tags, self.update_dofs)) {
        return_value = true;
      }
    }
    return return_value
  ...
  self.set_config = function(conf) {
    if (conf != undefined && conf != '') {
      ...
      self.tags = ['*'];
      self.tags = self.tags.concat(self.name.split(' '));

      self.grid_x = self._config.grid.x;
      self.grid_y = self._config.grid.y;

      self.type = self._config.type;
      self.tags = self.tags.concat(self.type.split(' '));

      self.actual_width = self._config.size.width;
      self.actual_height = self._config.size.height;

      self.code = svg_codes[self.type];
      ...
      if (self._config.dofs != undefined) {
        for (var i = 0; i < self._config.dofs.length; i++) {
          var conf_dof = self._config.dofs[i];
          var newdof = new Dof(conf_dof.name);
          if (conf_dof.servo != undefined) {
            newdof.setServo(conf_dof.servo.pin, conf_dof.servo.mid, conf_dof.servo.min, conf_dof.servo.max)
          }
          newdof.tags = newdof.tags.concat(self.tags);
...
var VirtualModel = function() {
  ...
  self.init = function() {
      console.log("init virtual model");
    // robot_data = ko.toJSON(robot_data, null, 2);

    // Load svgs from files
    var index = 0;
    svg_codes = {};
    for (var svg_type in robot_data.svg_urls) {
      index++;
      $.ajax({
        type: 'GET',
        url: robot_data.svg_urls[svg_type],
        data: {
          type: svg_type
        },
        success: function(data) {
          index--;
          svg_codes[this.url.split('=')[1]] = data.childNodes[0].outerHTML;
          if (index == 0) {
            self.set_config(robot_data);
          }
        }
      });
    }
  ...
  self.add_module = function(mod_type, config) {
    var mod = new modules_definition[mod_type](config);
  ...
  self.resize = function() {
    ...
    if (virtualModel == undefined) { return; }
  ...
  self.update_dofs = function(dofs) {
    ...
    for (var i = 0; i < dofs.length; i++) {
      ...
      for (var j = 0; j < self.modules.length; j++) {
        var mod = self.modules[j];
        mod.set_dof_tags(dof.value, dof.tags);
      }
    }
  ...
  self.update_sound = function(type, msg) {
    ...
    if (type == 'tts') {
      ...
      $('.robot_sound').html('<span class="fa ' + icon + '"></span> ' + msg.name);
    }


    // var sound_src = '/sound/?t=' + type + '&f=' + msg;
    var sound_src = msg.url;
...
let main_svg, virtualModel, svg_codes;
svg_codes = {};
if (robot_data != undefined) {
